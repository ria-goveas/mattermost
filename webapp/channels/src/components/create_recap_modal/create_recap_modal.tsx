...
const CreateRecapModal = ({onExited}: Props) => {
    ...
    const teamUrl = teamMatch?.pathnameBase ?? '';
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            onExited();
            navigate(`${teamUrl}/recaps`);
        } catch {
            setError(formatMessage({id: 'recaps.modal.error.createFailed', defaultMessage: 'Failed to create recap. Please try again.'}));
            setIsSubmitting(false);
        }
    }, [selectedChannelIds, currentUserId, selectedBotId, dispatch, onExited, navigate, teamUrl, formatMessage, recapName]);
